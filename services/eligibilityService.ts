...
export const calculateEligibility = async (answers: UserAnswers): Promise<EligibilityResult> => {
  ...
  if (answers.heatingType === HeatingType.HEAT_PUMP) {
      ...
      pac101.reasons.push('reason.fail_elec_heating');
       ...
       if (answers.ownerStatus !== 'owner') { // Strictly requires owner status
           pac101.reasons.push('reason.fail_owner_required');
       } else {
           pac101.reasons.push('reason.ok_owner');
       }
  ...
  if (answers.heatingType !== HeatingType.HEAT_PUMP) {
      ...
      if (answers.ownerStatus !== 'owner') { // SSC also strictly requires owner
          ssc.reasons.push('reason.fail_owner_required');
      } else {
          ssc.reasons.push('reason.ok_owner');
      }
  }
