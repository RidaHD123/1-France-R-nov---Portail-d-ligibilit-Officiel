...
function App() {
  ...
  const isCurrentAnswerValid = () => {
      ...
      if (currentQuestion.type === 'number') {
          // Check for NaN and bounds for number inputs
          return typeof val === 'number' && !isNaN(val) && val >= (currentQuestion.min || 0) && val <= (currentQuestion.max || 999999999);
      }
                  ...
                  <div className="mt-4">
                      <div className="flex items-center border-2 border-slate-300 rounded-xl focus-within:border-gov-blue focus-within:ring-4 focus-within:ring-blue-100 transition-all overflow-hidden bg-white">
                          <input
                              ...
                              value={val?.toString() ?? ''}
                              onChange={(e) => handleAnswer(q.type === 'number' ? Number(e.target.value) : e.target.value, q.id)}
